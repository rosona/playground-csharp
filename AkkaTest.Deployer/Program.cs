    ...
    class Program
    {
        ...
        class HelloActor : ReceiveActor
        {
            ...
            public HelloActor(IActorRef remoteActor)
            {
                ...
                Receive<Hello>(hello => { Console.WriteLine("Received {1} from {0}", Sender, hello.Message); });

                Receive<SayHello>(sayHello => { _remoteActor.Tell(new Hello("hello" + _helloCounter++)); });
        ...
        static void Main(string[] args)
        {
                ...
                }")))
            {
                ...
                var remoteEcho1 =
                    system.ActorOf(Props.Create(() => new EchoActor()), "remoteecho"); //deploy remotely via config
                var remoteEcho2 =
                    system.ActorOf(
